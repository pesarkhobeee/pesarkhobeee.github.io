...
XML_ReaderByWebservice("getWordingTranslation_Xml", "Localization", "");
...
function googleTrackEvent(title, link, isIndex) {
    try {
        ...
        switch (title) {
            case "HotProducts":
                var TrackBanner = "";
                /*if (typeof ($("img", link).attr("src")) == "undefined") {
                    var TrackBannerImg = $(link).parents("div .hot_product_cell").find("a");
                    TrackBanner = $("img", TrackBannerImg).attr("src");
                }
                else TrackBanner = $("img", link).attr("src");
                */
                ...
                break;
            case "Banner":
                /*var TrackBanner = $("img", link).attr("src");
                var index_pic = TrackBanner.indexOf("/banners/");
                TrackBanner = TrackBanner.substr(index_pic + 8)
                var pageView = TrackBanner;
                break;*/
                var TrackBanner = $(link).attr("alt");
                var pageView = TrackBanner;
                break;
            case "Spotlight":
            case "Feature":
        ...
        //var websitecode = $("GoogleTrackingCode", sysWording).text();
        //var pageTracker = _gat._getTracker(websitecode);
        //_gat._anonymizeIp(); //20110218 Add for Privacy Policy - Remove the last octet of the IP address
        ga('set', 'anonymizeIP', true);
        if (currentProductGroup == "" && title == "HeadNews") isIndex = true;
        if (isIndex == true) {
            //pageTracker._trackEvent("Index", title, pageView);
            ga('send', 'event', 'Index', title, pageView);
        }
        else {
            //pageTracker._trackEvent(currentProductGroup, title, pageView);
            ga('send', 'event', currentProductGroup, title, pageView);
        }
        ...
        if (title != "Product Comparison" && title != "Video" && link.title != "Recently_Viewed" && link.title != "Compare_List" &&
            ...
            //else setTimeout(function () { window.open(link.href); }, 100);
...
function googleTrackFamilySite(link, position) {
    try {
        ...
        var pageView = (link.title == "") ? link.innerHTML : link.title;
        //_gat._anonymizeIp(); //20110218 Add for Privacy Policy - Remove the last octet of the IP address
        //pageTracker._trackEvent('Family Site', position, pageView);
        ga('set', 'anonymizeIP', true);
        ga('send', 'event', 'Family Site', position, pageView);
...
function trackOverview(hashId, tab) {
    try {
       ...
       // var pageView = (link.title == "") ? link.innerHTML : link.title;
        //_gat._anonymizeIp();
        //pageTracker._trackEvent('Overview', hashId, tab);
        ga('send', 'event', 'Overview', hashId, tab);
    } catch (err) { }
}
function trackSpecialOverview(hashId, tab) {
    try {
      ...
      //  var pageView = (link.title == "") ? link.innerHTML : link.title;
        //_gat._anonymizeIp();
        //pageTracker._trackEvent('SpecialOverview', hashId, tab);
        ga('set', 'anonymizeIP', true);
        ga('send', 'event', 'SpecialOverview', hashId, tab);
    } catch (err) { }
}
//----2013 /05/24 @end-----
function googleTrackProduct(TrackTab) {
    try {
        ...
        var currentProductGroup = (isMultipleLanguageWebsite() || isApplicationPathSite()) ? window.location.pathname.split('/')[2] : window.location.pathname.split('/')[1];
        //pageTracker._trackEvent(currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), TrackTab);
        ga('send', 'event', currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), TrackTab);
        if (TrackTab != "Microsite" && TrackTab != "BuyNow" && TrackTab != "AddCompare" && TrackTab != "WTB_OnlineRetailer") {
        ...
        switch (title) {
            ...
            case "Video":
                ...
                if (link != '' && (title == "Gallery" || title == "Video")) {
                    ...
                    //pageTracker._trackEvent(currentProductGroup + " - Product", pageView.replace("/" + currentProductGroup, ""), title + "/" + TrackPic);
                    ga('send', 'event', currentProductGroup + " - Product", pageView.replace("/" + currentProductGroup, ""), title + "/" + TrackPic);
                    ...
                    if (title != "Gallery") {
                        //pageTracker._trackEvent(currentProductGroup + " - Product", pageView.replace("/" + currentProductGroup, ""), title);
                        ga('send', 'event', currentProductGroup + " - Product", pageView.replace("/" + currentProductGroup, ""), title);
                    }
                    //pageTracker._trackPageview(pageView + title);
                    ga('send', 'pageview');
                }
                break;
            case "WTB_OnlineRetailer":
                ...
                var pathname = window.location.pathname;
                var currentProductGroup = (isMultipleLanguageWebsite() || isApplicationPathSite()) ? window.location.pathname.split('/')[2] : window.location.pathname.split('/')[1];
                //pageTracker._trackEvent(currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), "WTB_OnlineRetailer - " + link.title);
                ga('send', 'event', currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), "WTB_OnlineRetailer - " + link.title);
                if (link.target == "_self") setTimeout('document.location = "' + link.href + '"', 100);
                break;
            case "WTB_Dealer":
                //var websitecode = $("GoogleTrackingCode", sysWording).text();
                ...
                var pathname = window.location.pathname;
                var currentProductGroup = (isMultipleLanguageWebsite() || isApplicationPathSite()) ? window.location.pathname.split('/')[2] : window.location.pathname.split('/')[1];
                //pageTracker._trackEvent(currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), "WTB_Dealer");
                ga('send', 'event', currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), "WTB_Dealer");
                break;
            case "Related_Item":
                //var websitecode = $("GoogleTrackingCode", sysWording).text();
                //var pageTracker = _gat._getTracker(websitecode);
                var pathname = window.location.pathname;
                var currentProductGroup = (isMultipleLanguageWebsite() || isApplicationPathSite()) ? window.location.pathname.split('/')[2] : window.location.pathname.split('/')[1];
                //pageTracker._trackEvent(currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), "Related_Item");
                ga('send', 'event', currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), "Related_Item");
            ...
            case "Similar_Product":
                ...
                var pathname = window.location.pathname;
                var currentProductGroup = (isMultipleLanguageWebsite() || isApplicationPathSite()) ? window.location.pathname.split('/')[2] : window.location.pathname.split('/')[1];
                //pageTracker._trackEvent(currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), "Similar_Product");
                ga('send', 'event', currentProductGroup + " - Product", pathname.replace("/" + currentProductGroup, ""), "Similar_Product");
...
var mobieChangeVersion = function (e) {
    e.preventDefault();
    //1:mobile 2:desktop  
    e.data.v == 1 ? setStorage('asus-version', 'mobile') : setStorage('asus-version', 'desktop');
...
$(document).ready(function () {
    ...
    if (checkMobile()) {
        ...
        setVg();
        changeMeta(getStorage('asus-version'));
    }
